...
#region Draw Arc
	...
	global.arcBufferFormat = vertex_format_end();
	global.arcBuffer = vertex_create_buffer();

	///@func draw_arc(x1, y1, x2, y2, color, texture, thickness, height, segments, startZ*, endZ*)
	function draw_arc(_x1, _y1, _x2, _y2, _color, _texture, _thickness, _height, _segments, _startZ, _endZ){
	  if(false) argument[0] = false; //Optional Argument Warning Supression
	  _startZ = _startZ != undefined ? _startZ : 0;
	  _endZ = _endZ != undefined ? _endZ : 0;
	  var _dir = point_direction(_x1,_y1,_x2, _y2);
	  var _len = point_distance(_x1,_y1,_x2,_y2);
	  var _xLen = lengthdir_x(_len,_dir)
	  var _yLen = lengthdir_y(_len,_dir)

	  var _leftOff = {x: lengthdir_x(_thickness,_dir+90), y: lengthdir_y(_thickness,_dir+90)}
	  var _rightOff = {x: lengthdir_x(_thickness,_dir-90), y: lengthdir_y(_thickness,_dir-90)}

	  var _uvs = texture_get_uvs(_texture);
  
	  vertex_begin(global.arcBuffer, global.arcBufferFormat)
	  for(var _i = 0; _i <= _segments; _i++) {
	    var _pos = (_dir < 180) ? 1 - (_i/_segments) : _i / _segments;
	    var _x = lerp(_x1,_x1+_xLen,_pos)
	    var _y = lerp(_y1,_y1+_yLen,_pos);
	    var _z = -sin(pi*_pos) * _height-lerp(_startZ,_endZ,_pos);
  
	    vertex_position(global.arcBuffer,_x + _leftOff.x,_y+_z+_leftOff.y)
	    vertex_texcoord(global.arcBuffer,lerp(_uvs[0],_uvs[2],_pos),_uvs[1])
	    vertex_color(global.arcBuffer,_color,1)
  
	    vertex_position(global.arcBuffer,_x+_rightOff.x,_y+_z+_rightOff.y)
	    vertex_texcoord(global.arcBuffer,lerp(_uvs[0],_uvs[2],_pos),_uvs[3])
	    vertex_color(global.arcBuffer,_color,1)
	  }
	  vertex_end(global.arcBuffer);

	  vertex_submit(global.arcBuffer,pr_trianglestrip, _texture);
	}
#endregion
...
function tile_meeting_precise(_x, _y, _layer, _checker) {
	var _tm = layer_tilemap_get_id(_layer);
